...
- `2026-08-01`：YouTube 订阅频道可按 `YT1`/`YT2` 展开——说「展开 YT1」或「详细解释 YT2」后按 skill 内 `references/podcast-translation-rules.md` 生成完整三部双语文档
...
日报只是第一层筛选。看完以后你可以继续让 Agent 展开任意一条内容，尤其是长播客和 YouTube：

- “展开第 2 个播客” / “展开 P2”
- “把 Vercel agents 这期做一个 breakdown”
- “这期播客按核心观点、论证链、关键引用、投资含义展开”
- “展开 YT1” / “展开 YT1, YT2” / “详细解释 YT2”

如果该播客有全文字幕，日报会标记为可展开；只有你明确要求“展开 P2”后，Agent 才会按 `guid` 拉取这一期全文，而不是每天预先下载所有字幕。

YouTube 订阅频道（`config/sources.json` → `youtube.channels`）在日报里编号为 `YT1`/`YT2`。说“展开 YT1”或“详细解释 YT2”后，Agent 会按需拉取该视频字幕，并按 skill 内 `references/podcast-translation-rules.md` 生成完整三部双语文档（核心要点索引 + 英汉对照全文 + 批判与投资分析），保存到当前项目下的 `podcast/`（没有则自动新建）。
...
> **帮我安装 https://github.com/newday1/ai-signal**
...
```bash
# OpenClaw
git clone https://github.com/newday1/ai-signal.git ~/skills/ai-signal
cd ~/skills/ai-signal/scripts && pip install -r ../requirements.txt

# Claude Code
git clone https://github.com/newday1/ai-signal.git ~/.claude/skills/ai-signal
cd ~/.claude/skills/ai-signal/scripts && pip install -r ../requirements.txt

# 其他
git clone https://github.com/newday1/ai-signal.git
...
```bash
git clone https://gh-proxy.com/https://github.com/newday1/ai-signal.git
# 或
git clone https://ghfast.top/https://github.com/newday1/ai-signal.git
```
...
[GitHub](https://github.com/newday1/ai-signal)
